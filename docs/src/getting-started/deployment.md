...
An async handler that reads the lazy `request.user` blocks its worker thread until the ORM pool loads the user. In an async handler, use `await request.auser()`. It loads the user on the request lane or on the ORM pool while the worker serves other requests.
