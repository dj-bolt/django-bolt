...
- **Dual dispatch** (`src/handler.rs`): sync path (`dispatch_sync.call1()` in one GIL block, `DispatchOutcome::Ready`) when `can_sync_dispatch`; `SyncResult` for sync handlers returning stream/file; async path (`Pending`) submits to the `WorkerLoop`. Trivially-async handlers (no `GET_AWAITABLE` in bytecode) get a `_sync_executor`. An async handler whose source reads `request.user` gets `_with_preloaded_user` (awaits `request.auser()` first) and no `_sync_executor`, unless `preload_is_faster()` is false (GIL + only SQLite: GIL contention makes the async hand-off slower) and a blocking read works (no async-only `get_user`, no `AuthenticationMiddleware`). A handler whose dependencies are all sync gets a sync injector, so it keeps sync and lane dispatch.
...
- **Request lanes** (`crates/bolt-core/src/lane.rs`): a route with Django middleware gets one Rust-owned thread per request, so thread-local state (django-tenants) stays with the request. Lanes are reused, size themselves, and stop after 10 s idle. A sync flow behind `DjangoMiddlewareStack` runs complete on the lane (`can_lane_dispatch`; `_dispatch_sync` drives `_dispatch` with no suspend, each hop inline). An async flow uses a `RequestLane` as the asgiref executor of its `ThreadSensitiveContext`. A new `await` in the dispatch path must stay inline on a lane (`in_lane_mode()`). A sync `request.user` read on the loop of a lane request (`run_orm_blocking`) blocks on the executor asgiref would pick: `AsyncToSync.executors.current` first (the lane's `CurrentThreadExecutor` while it waits in `async_to_sync`), then the `RequestLane`. Submitting straight to the `RequestLane` deadlocks there. A thread with no loop that is not the lane raises `LaneAffinityError`; it must not wait for the lane.
