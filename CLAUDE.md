...
1. **Rust Layer (`src/`)**
   ...
   - `crates/bolt-loop/` - workspace crate: the asyncio `WorkerLoop` machinery (`scheduler.rs` pump/timers/signals + `WorkerLoopScheduler` pyclass, `fd.rs` Tokio-reactor fd watchers, `dispatch.rs` run-a-callable-on-the-loop). Its Python half is `python/django_bolt/_worker_loop.py`; `lib.rs` wires them with `bolt_loop::init("django_bolt._worker_loop")` + `bolt_loop::register(m)`
