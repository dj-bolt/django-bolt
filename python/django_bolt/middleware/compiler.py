...
def _extract_type_hints_from_field(field: Any, claims: dict[str, tuple[str, int]]) -> None:
    ...
    for label, wire_name, annotation in entries:
        key = wire_name.lower().replace("_", "-") if field.source == "header" else wire_name
...
def add_optimization_flags_to_metadata(metadata: dict[str, Any] | None, handler_meta: dict[str, Any]) -> dict[str, Any]:
    ...
    # Path and query share one map in Rust. Headers, cookies and forms each have their own map.
