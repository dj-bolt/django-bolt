...
event loop thread first. The loader checks for a running loop when it runs.
A thread with a running loop cannot run the ORM inline, so the query goes
to a worker thread. A request lane runs the query inline, which keeps its
thread affinity.
"""
...
# (backend_name) -> resolved loader (user_id, auth_context) -> user,
# or None when the backend has no user resolution. Built at registration time.
_resolved_loader_registry: dict[str, Callable[[str, dict | None], Any] | None] = {}
...
def resolve_user_loader(backend: Any) -> Callable[[str, dict | None], Any] | None:
    """
    ...
    whether a running event loop prevents an inline query.
    """
    cls = type(backend)
    ...
    if custom_async:
        ...
        def load_via_async(user_id: str, auth_context: dict | None) -> Any:
            # asyncio.run needs a thread without a running loop — always cross,
            # regardless of the calling thread. The shim goes to the generic
            ...
            # QuerySet evaluation behind it. A calling lane processes callbacks
            # while it waits, so nested database work can return to that lane.
...
def load_user_sync(
    ...
    if backend_name in _auth_backend_registry:
        ...
        if loader is None:
            ...
            return None
        return loader(user_id, auth_context)
