...
- **Bolt loads `request.user` before an async handler that reads it** - Bolt reads the source of each handler at registration. When an async handler reads `request.user`, Bolt loads the user with `await request.auser()` before the handler runs. The read then does not block the event loop. Measured with PostgreSQL, such a route serves 12 to 28% more requests per second, and its p99 latency is about 30% lower. The rule does not depend on the database, so tests and production take the same path. On free-threaded Python, the load serves about twice as many requests per second as a blocking read. With the GIL and a local SQLite database, a route that reads `request.user` serves about 30% fewer requests per second than a blocking read, because the loop thread and the ORM thread compete for the GIL. The event loop still serves other requests while the user loads, so another route on the same server serves about twice as many requests. The load also makes a backend with only an async `get_user` work in such a handler, and the session user of `AuthenticationMiddleware`, which raised `SynchronousOnlyOperation` before. A handler that does not read `request.user` loads nothing.
...
- **BREAKING: sync `request.user` raises for a backend with only an async `get_user`** - Bolt ran such a `get_user` with `asyncio.run` on a pool thread. When an async handler reads `request.user` in its own source, Bolt now loads the user first, so the read works. Any other sync read of `request.user` raises `RuntimeError`, and the message names the fix. To migrate, define `get_user_sync` on the backend, or read the user with `await request.auser()`.
...
- **The parameters of a dependency bind like the parameters of a handler** - A dependency used its own copy of the parameter binding, which had these bugs:
  ...
  A dependency now uses the same extractors as a handler. It gets the same names, aliases, defaults, type conversion and 422 errors. Its form fields, file constraints and upload cleanup work as for a handler. A handler and a dependency that take the same body type share one decode of the body. Other body types decode separately. The OpenAPI schema of the route shows its parameters and its body. When a handler parameter and a dependency parameter have the same name, the handler parameter wins. A sync handler uses the sync injector only when no dependency in its tree is async. A nested dependency counts in its sync form, so `def org(user=Depends(get_current_user))` keeps a sync handler on the sync injector.
- **A revoked token fails the WebSocket handshake** - A `revoked_token_handler` or a `revocation_store` applied only to HTTP routes. A WebSocket route with the same backend accepted a revoked token. The handshake now awaits the revocation check before the upgrade, and a revoked token gets `401`. `WebSocketTestClient` runs the same check. An open connection stays open after its token is revoked.
...
- **`request.user` loads on the thread that reads it** - Bolt chose the thread of the user query at registration, from the type of the handler. A trivially-async handler read the user on the sync fast path, where Rust installs an event loop, and Django raised `SynchronousOnlyOperation`. The loader now checks the thread when the read occurs. A thread with no running event loop runs the query inline, so a lane keeps its thread-local state. A thread with a running loop sends the query to the ORM pool, and the loop waits for it. This also applies with `DJANGO_ALLOW_ASYNC_UNSAFE=1`, where the query ran inline before. On the event loop of a request with Django middleware, the query runs on the request lane (see above).
