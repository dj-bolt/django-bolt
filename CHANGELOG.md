...
- **BREAKING: sync `request.user` raises on the event loop of a request with Django middleware** - Such a request keeps its thread-local state on its lane, for example a tenant schema. A sync read of `request.user` on the event loop sent the query to the ORM pool, where that state is not set. The query could thus read the wrong schema, and a handler on the lane then used that user too. The read now raises `RuntimeError`, as Django raises `SynchronousOnlyOperation` under ASGI. This applies to an async handler and an async Python middleware behind Django middleware, and to a coroutine that `async_to_sync` runs. To migrate, use `await request.auser()`. A request with no Django middleware still loads the user on the ORM pool.
