...
- **MCP spec 2026-07-28 support: bolt-mcp rebuilt on the official MCP Rust SDK (rmcp)** - The MCP protocol core moves from a hand-rolled Python implementation into django-bolt's Rust extension (`src/mcp/`), built on [rmcp](https://github.com/modelcontextprotocol/rust-sdk) 3.1. The `/mcp` endpoint is now **dual-era**: 2026-07-28 clients are served statelessly (per-request `_meta`, `server/discover`, SEP-2243 routing-header validation with `HeaderMismatch` -32020, SEP-2549 `ttlMs`/`cacheScope` caching hints, multi-worker safe), while legacy session clients (`initialize` + `Mcp-Session-Id`, 2024-11-05 through 2025-11-25) keep working on the same endpoint. Elicitation on modern clients uses **MRTR** (Multi Round-Trip Requests): `ctx.elicit()` ends the call with an `input_required` result plus an HMAC-sealed `requestState` bound to the tool, arguments, and principal; the client retries with the answers and the tool replays deterministically (`ctx.is_replay`, `ctx.elicit(key=...)`). Catalog requests (`tools/list`, `server/discover`, `resources/list`, `prompts/list`) are answered entirely in Rust from a registry compiled at mount time, per-tool guards are evaluated in Rust (both call denial and per-principal `tools/list` filtering), and modern-era logging honors the per-request `io.modelcontextprotocol/logLevel` gate. New knobs: `MCP(title=, instructions=, website_url=, icons=, list_ttl_ms=, list_cache_scope=)`, `@mcp.tool(annotations=, icons=, output_schema="auto")`, `mount_mcp(allowed_hosts=, allowed_origins=)` for DNS-rebinding protection. The built-in OAuth AS adds RFC 9207 `iss` on authorization responses and honors DCR `application_type` ("web"/"native"). MCP tool calls now get real cancellation: rmcp cancels the dispatch when the client closes the response stream.
- **BREAKING (bolt-mcp 0.2)**: bolt-mcp requires django-bolt >= 0.10; `bolt_mcp.transport`, `bolt_mcp.sessions`, `bolt_mcp.types`, `MCP.dispatch`, and `McpError` are removed (rmcp owns the wire protocol); `MCP(stateless=)`/`MCP(json_response=)` now shape only legacy-era behavior, and `json_response=True` implies sessionless serving; legacy `notifications/message` logs are delivered on the session's standalone GET stream (not the POST stream); some transport error statuses/texts differ (e.g. malformed bodies are rejected with 415, missing sessions with 422).

- **`Requires` exclusion and custom denial messages** - `Requires(claim, none_of=[...])` is the NOR quantifier: the claim must match none of the listed values, for excluding a few values instead of enumerating every allowed one. Positional values, `all_of`, and `none_of` fill the same quantifier slot, so exactly one may be given; guards are cumulative, which is how "has X but not Y" is spelled (two guards). An unauthenticated request fails *every* `Requires` — including a `none_of` — with a 401: a caller carrying no claims matches nothing, so without this it would satisfy an exclusion and walk straight through the guard. `message="..."` sets the `detail` returned with the 403; it is serialized to response bytes once at registration, so a custom message costs nothing per request, and it is never used for the 401 — an unauthenticated caller is not told why it would have been denied. (#273)
- **JWKS support for external identity providers** - `JWTAuthentication(jwks_url=...)` fetches a provider's JSON Web Key Set (Clerk, Auth0, Okta, ...) at server startup; the set is parsed into per-`kid` decoding keys in Rust and the token's `kid` header selects the key on each request. A `jwks=` dict/JSON parameter supplies the set directly (tests, air-gapped deployments). Rotation to a new `kid` is picked up on the next server start. (#255)
- **Access + refresh token lifecycle** - `create_token_pair(user, ...)` and `rotate_refresh_token(...)` implement the dual-token flow: short-lived access tokens validated entirely in Rust, long-lived refresh tokens used only at a rotation endpoint. Both rotation modes (full rotation with reuse detection, and access-only re-issue) are supported. Issuance is auth-method-agnostic (password, magic-link, OAuth, internal handoff) with an optional RFC 8176 `amr` claim. The claim schema (`typ`, `jti`, `ver`, `fam`, `oat`) is fixed from the first release so future features need no breaking claim changes. `set_token_cookies(response, pair)` attaches the pair as `HttpOnly`/`Secure`/`SameSite=Lax` cookies; pass `refresh_path="/auth/refresh"` (your rotation endpoint) to path-scope the refresh cookie so it never rides along on ordinary API calls. Extra `claims=` cannot override the reserved lifecycle claims (`ValueError`), and custom claims are not carried across rotation automatically — re-derive and pass them via `claims=` at each rotation. (#239)
- **Symmetric token-type enforcement** - `JWTAuthentication(token_type="refresh")` requires `typ:"refresh"` (for rotation endpoints), and default access routes now reject `typ:"refresh"` tokens — both enforced in Rust before the Python handler runs, so a refresh token can never authenticate a normal endpoint. (#239)
- **Bulk revocation primitives** - Revocation stores gain `get_user_version`/`bump_user_version` (global "log out everywhere" via a per-user token version, O(1), no key scanning) and `revoke_family`/`is_family_revoked` (refresh-token reuse detection), implemented for `InMemoryRevocation` and `DjangoCacheRevocation`. A version bump invalidates every earlier refresh token at rotation (`rotate_refresh_token` rejects stale `ver`); already-issued access tokens are not re-checked against the store and remain valid until `exp` — keep access TTLs short. (#239)
- **CSRF protection for cookie-authenticated routes** - When a JWT backend reads its token from a `cookie`, unsafe-method (state-changing) requests **that carry that cookie** are checked for cross-site origin in Rust (`Sec-Fetch-Site`, falling back to an `Origin`/`Referer` host comparison), closing the gap left by bolt bypassing Django's `CsrfViewMiddleware`. Requests credentialed another way (a header backend on a mixed-backend route) or carrying no auth cookie are exempt — browser-attached cookies are the CSRF vector. Enabled by default for cookie backends; opt out with `csrf=False`. (#239)
- **Explicit `public_key=` and `leeway=` parameters** - `JWTAuthentication(public_key=...)` is the clear spelling for asymmetric verification keys (passing a PEM via `secret=` still works). `leeway=` configures the `exp`/`nbf` clock-skew tolerance (default 60s). (#261)
- **`RS256` verification proven over a real server** - New `server_integration` tests spin up `runbolt` with an RSA public key and assert valid, tampered, missing, and non-string-header-extra tokens behave correctly, plus that an invalid PEM aborts startup.

### Performance

- **Hot-path dispatch overhaul** - Async HTTP dispatch now runs on a process-lived worker-local asyncio loop (`WorkerLoop`) serviced by a persistent Tokio pump, eliminating per-request Task scheduling and cross-thread wakeups — the single biggest overhead for simple async handlers. Rust-side argument prebinding is extended to optional scalar params, unlocking the sync-dispatch bypass (~6-12 µs per request) for path/query-param routes; eligible sync routes return a bare encoded JSON body instead of the full response tuple (no tuple construction in Python, no parsing in Rust); typed params (`datetime`, `date`, `time`, `UUID`) are constructed directly in Rust with no string round-trip; each request acquires the GIL exactly once for dispatch. A pass of allocation eliminations rounds it out: in-place path-param URL decoding, static method names, interned route-declared param names, and access-log formatting outside the GIL. Async handlers returning QuerySets are now evaluated on a bounded, database-vendor-aware ORM executor pool (`DJANGO_BOLT_ORM_THREADS`) instead of a single shared thread — parallel, but with a bounded connection count. (#268)
- **Layered performance measurement infra** - criterion benches for pure-Rust hot functions (`just bench-rust`), pytest-benchmark micro-benches for injectors/deps/serialization (`just bench-micro`), and a deterministic regression gate on per-endpoint RPS *and* p99 latency (`just bench-gate`), documented in `docs/PROFILING.md`. (#268)

### Changed

- **Vite-style dev server startup** - `runbolt --dev` prints a compact brand banner with arrow-prefixed URL rows and honest "ready in N ms" timing (measured from worker spawn, so it covers interpreter startup and `django.setup()`, not just route registration), and the dev supervisor emits timestamped, colored `HH:MM:SS [bolt] ...` reload lines. Color follows Django's `--no-color`/`--force-color` plus `NO_COLOR` and `TERM=dumb`, with ASCII glyph fallback for piped and Windows-console output. (#272)
- **BREAKING for cookie-JWT deployments**: routes using `JWTAuthentication(cookie=...)` (added in 0.9.1) now enforce the CSRF origin check by default on unsafe methods when the auth cookie is present. Browser clients are unaffected (they send `Sec-Fetch-Site` or `Origin`). Non-browser clients that authenticate via the cookie **without** sending any origin signal will start receiving 403 — either send `Sec-Fetch-Site: none`/a matching `Origin`, switch to header-sourced tokens, or opt out with `csrf=False`.

### Fixed

- **`--dev` reload covers the project root and the api import graph** - The dev supervisor never imported the api module, so its watch list held only `django.setup()`'s import graph plus `api.py` itself — edits to helpers imported by `api.py` never reloaded, and newly created modules were invisible. The supervisor now primes the watch list by importing the modules autodiscovery will import, always watches the project root recursively (new files and packages reload without restarting the dev server), and gains a repeatable `--reload-dir` flag for code outside both. `python -m pkg.manage` invocations are preserved when respawning the worker, and a failed *first* start exits with the worker's code (port taken, broken settings) instead of waiting forever for a change that cannot fix it. (#272)
- **Streaming and WebSocket coroutines run on the WorkerLoop** - Streaming forwarders and WebSocket handlers were scheduled on the startup selector loop while async HTTP dispatch runs on the WorkerLoop. Asyncio primitives shared across that split never deliver — `Future.set_result` wakes waiters with a same-loop `call_soon`, which does not rouse a foreign loop's selector — so a WebSocket handler awaiting a future resolved by an HTTP request would hang forever. Both now run on the WorkerLoop; mounted ASGI apps deliberately stay on the selector loop, since a mount cannot share an asyncio primitive with a Bolt handler. (#273)
- **`sync_to_thread` keyword arguments actually bind** - kwargs are now bound via `functools.partial`; `run_in_executor` never accepted keyword arguments, a latent bug surfaced by keyword-bound Rust prebinding. (#268)
