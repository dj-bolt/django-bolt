...
- **`await request.auser()` loads the user of Bolt auth without a block on the event loop** - `request.auser()` returned the lazy `request.user` object, so the first attribute read still forced a sync query on the event loop thread. It now awaits the async loader of the backend. An async `get_user` runs as a coroutine of the request. A sync `get_user_sync` runs on the lane of the request, or on the ORM pool. `request.user` then returns the same user with no second query. `auser()` returns the loaded user, not the lazy proxy.
- **Sync access to `request.user` raises for a backend with an async `get_user` only** - Bolt ran such a `get_user` with `asyncio.run` on a pool thread. A sync read of `request.user` now raises `RuntimeError` that names `await request.auser()`. Define `get_user_sync` when sync code reads `request.user`. `load_user()` returns `None` for a backend with no user resolution.
- **`resolve_user_loader()` returns a `(sync loader, async loader)` pair** - The async loader serves `request.auser()`.
...
- **`DjangoMiddleware` keeps an async-only middleware on the event loop** - A `MiddlewareMixin` subclass can set `sync_capable = False` and return an awaitable from a plain `def __call__`, for example `asyncio.shield(...)`. Such a middleware ran on a thread with no event loop, and the request failed. Bolt now reads `sync_capable` when it selects the call path. It awaits the response of such a middleware on the event loop, and it accepts any awaitable, a Future included.
- **`DjangoMiddlewareStack` gives an async-only middleware an async `get_response`** - The stack built one sync chain for all `__call__` middleware. An async-only middleware that awaited `get_response` in that chain failed. The stack now awaits such a middleware on the event loop and gives it an async `get_response`, as Django does. The hooks before it and the handler after it still run on the lane of the request.
- **A lazy `request.user` loads on the thread that forces it** - A sync handler behind Django middleware runs on a request lane. Bolt marked its user loader for a thread with no event loop at registration. An async Python middleware around that handler forced the user on the event loop first, and Django raised `SynchronousOnlyOperation`. The loader now checks the calling thread when it runs. A thread with a running event loop sends the query to a worker thread. A request lane runs the query inline, so the query keeps the thread affinity of the lane.
- **A user query awaited on the event loop runs on the lane of its request** - A lazy `request.user` forced on the event loop went to the ORM pool. It then missed the thread-local state of the Django middleware, for example a tenant schema. `await request.auser()` now sends the query to the lane of the request, as `sync_to_thread` does. A sync read of `request.user` on the event loop still blocks that loop and uses the ORM pool.
- **`request.user` works in a trivially-async handler on the sync fast path** - Rust installs the event loop around the sync fast path of an `async def` handler with no await. The user query ran inline there, and Django raised `SynchronousOnlyOperation`. The loader now sees the loop and sends the query to the ORM pool.
