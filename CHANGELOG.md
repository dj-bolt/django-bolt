...
- **Bolt loads `request.user` before an async handler that reads it** - Bolt reads the source of each handler at registration. When an async handler reads `request.user`, Bolt loads the user with `await request.auser()` before the handler runs. The read then does not block the event loop. This also loads a user whose backend has only an async `get_user`, and the session user of `AuthenticationMiddleware`, which raised `SynchronousOnlyOperation` before. A handler that does not read `request.user` loads nothing.
