...
- **micro-reflex state synchronization in Rust** - Per-session render caches, change diffing, and patch-frame JSON encoding moved from Python into `src/microreflex.rs` (`RxPage`/`RxSession`). Pages precompute their slot table (patch prefixes included) once at registration; per event, Python only evaluates slot values (user code) and Rust diffs against what that client last saw — comparing without copying unchanged strings — and emits the complete wire frame via serde_json. Typical pages diff+encode ~2-6x faster than the prior Python path; user state objects and event handlers remain Python by design.
- **micro-reflex proof of concept** - `django_bolt.microreflex` runs Reflex's developer syntax (`rx.State`, component functions, event handlers, `@rx.var` computed vars, `rx.cond`/`rx.foreach`, generator handlers that `yield` intermediate updates) on django-bolt's Rust engine with **pure HTML + a dependency-free vanilla-JS runtime** instead of React/Next.js. Pages are compiled once at registration into a constant HTML string served through the sync-dispatch fast path; interactivity flows over one Rust-handled WebSocket route as per-session slot diffs (`textContent`/`innerHTML`/attribute patches). Small Reflex apps drop in by changing only the import. In-process event round-trips measure ~90µs (~11k/s on one connection). See `docs/MICROREFLEX.md`.
...
- **Browser same-origin WebSocket handshakes no longer 403 without CORS config** - WebSocket origin validation assumed browsers omit `Origin` on same-origin connects; browsers always send it, so every browser WebSocket connection to an app without CORS configured was rejected. An `Origin` whose authority matches the request's `Host` header (default ports normalized) is now recognized as same-origin and allowed; cross-origin requests are still denied unless CORS allows them (fail-secure), in both the production server and the in-process `WebSocketTestClient` path.
