...
- **`DjangoMiddleware` keeps an async-only middleware on the event loop** - A `MiddlewareMixin` subclass with `sync_capable = False` and a plain `def __call__` that returns an awaitable, for example `asyncio.shield(...)`, ran on a thread with no event loop and the request failed. Bolt now reads `sync_capable` when it selects the call path. It awaits the response of such a middleware on the event loop, and it accepts any awaitable, a Future included.
...
- **A lazy `request.user` loads on the thread that forces it** - A sync handler behind Django middleware runs on a request lane. Bolt marked its user loader for a thread with no event loop at registration. An async Python middleware around that handler forced the user on the event loop first, and Django raised `SynchronousOnlyOperation`. The loader now checks the calling thread when it runs. A thread with a running event loop sends the query to the ORM pool. A request lane runs the query inline, so the query keeps the thread affinity of the lane. `load_user_sync()` no longer takes `is_async_context`.
