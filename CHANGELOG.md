...
- **The 422 body for a bad path or query parameter is valid JSON** - The error message repeats the value that the client sent. Bolt escaped only quotes and backslashes in it. A value with a control character, for example `?n=%0A` or `?n=%09` for an `int` parameter, then gave a body that JSON clients could not parse. Bolt now escapes all control characters (U+0000 to U+001F) as `\n`, `\t`, `\r`, or `\u00XX`.
