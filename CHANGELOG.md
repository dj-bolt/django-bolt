...
- **Typed parameters are converted by their wire name** - Rust looked up the type of a parameter by its Python name. The value arrives by its wire name, so a parameter whose wire name is different stayed a string. This applied to `Query(alias="p")`, `Path(alias=...)`, `Cookie(alias=...)`, `Form(alias=...)`, and to every typed header, because `x_count` arrives as `x-count`. A typed header struct field failed with 422 on a valid value. The same problem applied to WebSocket path and query parameters. Thus `page: Annotated[int, Query(alias="p")]` received `"3"` for `?p=3` and accepted `?p=abc`. Bolt now uses the wire name, so the handler gets `3` and `?p=abc` gets a 422.
- **A bad typed header or cookie value gets a 422** - An invalid `int` or `float` header or cookie value became `0` or `0.0`, and an invalid `bool` became `False`. Rust now checks the typed headers and cookies of a handler before it calls Python and returns 422. Headers and cookies have their own type maps, so a typed query parameter `id: int` does not change an unrelated `id` cookie.
- **Two types for one wire key fail at registration** - Two parameters can read the same wire key, for example `p: str` and `page: Annotated[int, Query(alias="p")]`. Rust converts a wire value one time, so one of them got the wrong type. Registration now raises `TypeError` when such parameters have different types. Parameters with the same type can share a key.
