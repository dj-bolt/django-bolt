...
- **A user query forced on the event loop runs on the lane of its request** - The query went to the ORM pool. It then missed the thread-local state of the Django middleware, for example a tenant schema. When the request owns a lane, the query now runs on that lane, as `sync_to_thread` does.
