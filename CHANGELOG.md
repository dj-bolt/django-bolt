...
- **Two types for one wire key fail at registration** - Two parameters can read the same key, for example `p: str` and `page: Annotated[int, Query(alias="p")]`. Rust converts a value one time, so one of them got the wrong type. Registration now raises `TypeError` when such parameters have different types. Parameters with the same type can share a key. Path and query parameters share one set of keys. Headers, cookies and form fields each have their own.
