...
- **`CurrentUser` parameter** - `def me(user: CurrentUser)` gives the authenticated user, as `Depends(get_current_user)` does. It works in sync and async handlers. For the type of your user model, make your own alias: `Annotated[User, Depends(get_current_user)]`.
- **Startup warning for a sync `request.user` read that raises** - Bolt reads the source of each handler when it registers the route. An async handler on a route with Django middleware that reads `request.user` gets a `RuntimeWarning` at startup. The warning names the handler and the line, and says to use `await request.auser()` or `CurrentUser`. A read in a nested function or a lambda, for example in `sync_to_thread`, does not get a warning.
...
- **BREAKING: sync `request.user` raises away from the lane of a request with Django middleware** - Such a request keeps its thread-local state on its lane, for example a tenant schema. A sync read of `request.user` on the event loop sent the query to the ORM pool, where that state is not set. A read on a thread from `asyncio.to_thread` ran the query on that thread, also without the state. The query could thus read the wrong schema, and a handler on the lane then used that user too. The read now raises `RuntimeError`, as Django raises `SynchronousOnlyOperation` under ASGI. This applies to an async handler and an async Python middleware behind Django middleware, and to a coroutine that `async_to_sync` runs. It also applies to indirect reads: `bool(request.user)`, a template that reads `{{ user }}`, and the debug toolbar. To migrate, use `await request.auser()`. The user is then cached, and later sync reads work. A request with no Django middleware still loads the user on the ORM pool.
