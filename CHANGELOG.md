...
- **Request lanes for Django middleware routes** - The thread of a request is a lane: a thread that Rust owns, that serves one request at a time, and that stays alive with its database connections between requests. A sync handler behind `DjangoMiddlewareStack` runs the complete request on one lane with no asyncio. An async handler sends its sync work to one lane. In an async request, one thread hop runs all hooks of a phase, which cut a request with four middleware from 707 to 514 microseconds in process. Lanes need no configuration: Bolt starts a lane when none is idle, and a lane that stays idle for 10 seconds closes its connections and stops (`DJANGO_BOLT_LANE_IDLE_SECONDS`). With django-tenants on PostgreSQL, one process, and 64 connections, a sync ORM route serves 1066 requests per second. django-ninja serves 795 on gunicorn with 32 threads and persistent connections. An async ORM route serves 914 against 310 for django-ninja on uvicorn.
