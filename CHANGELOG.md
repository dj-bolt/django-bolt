...
- **A sync `request.user` read on the event loop runs on the request lane** - A request with Django middleware keeps its thread-local state on its lane, for example a tenant schema. A sync read of `request.user` on the event loop sent the query to the ORM pool, where that state is not set. The query could thus read the wrong schema, and a handler on the lane then used that user too. The query now runs on the lane, and the event loop waits for it. Thus an async handler, an async Python middleware, a template that reads `{{ user }}`, and a coroutine that `async_to_sync` runs all get the user of the correct schema. When the lane waits for the event loop in `async_to_sync`, the query goes through its `CurrentThreadExecutor`, as asgiref sends thread-sensitive work, so the two do not deadlock. `await request.auser()` does not block the event loop and stays the faster form in async code.
