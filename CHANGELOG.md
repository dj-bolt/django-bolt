...
- **Dead database connections are dropped on error** - Bolt keeps Django connections open across requests. When a handler, a QuerySet evaluation, or the load of `request.user` on the executor pool raises, Bolt now runs Django's `close_if_unusable_or_obsolete()` on that thread, so a broken connection does not fail every later request on the same thread. Before, a PostgreSQL server that closed its connections left every authenticated request on the thread answering 500 `OperationalError: the connection is closed` until the process restarted. The check adds no cost on the success path. With the default settings, the request that meets the dead connection still fails, and the next one reconnects. When a database sets a positive `CONN_MAX_AGE` or `CONN_HEALTH_CHECKS`, the same check also runs before each executor call, so timed recycling and health checks work without `BOLT_EMIT_SIGNALS`, and a health check replaces a dead connection before the query, so no request fails. Neither setting is on by default. See [Database Connection Management](docs/src/topics/signals.md#database-connection-management).
