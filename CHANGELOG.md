...
- **`request.user` loads on the thread that reads it** - Bolt chose the thread of the user query at registration, from the type of the handler. That choice was wrong in two cases, and Django raised `SynchronousOnlyOperation`. In the first case, an async Python middleware read the user on the event loop around a sync handler on a lane. In the second case, a trivially-async handler read it on the sync fast path, where Rust installs an event loop. The loader now checks the thread when the read occurs. A thread with no running event loop runs the query inline, so a lane keeps its thread-local state. A thread with a running loop sends the query to the ORM pool, and the loop waits for it. This also applies with `DJANGO_ALLOW_ASYNC_UNSAFE=1`, where the query ran inline before.
