...
- **Bolt loads `request.user` before an async handler that reads it** - Bolt reads the source of each handler at registration. When an async handler reads `request.user`, Bolt loads the user with `await request.auser()` before the handler runs. The read then does not block the event loop. Measured with PostgreSQL, such a route serves 12 to 28% more requests per second, and its p99 latency is about 30% lower. With the GIL and only SQLite databases, Bolt does not load the user first. There, the loop thread and the ORM thread compete for the GIL during the async hand-off, and a blocking read is faster. On free-threaded Python, the load serves about twice as many requests per second as a blocking read. Bolt always loads the user first when a sync read cannot work. This applies to a backend with only an async `get_user`. It also applies to the session user of `AuthenticationMiddleware`, which raised `SynchronousOnlyOperation` before. A handler that does not read `request.user` loads nothing.
