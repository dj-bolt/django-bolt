...
- **Bulk revocation primitives** - Revocation stores gain `get_user_version`/`bump_user_version` (global "log out everywhere" via a per-user token version, O(1), no key scanning) and `revoke_family`/`is_family_revoked` (refresh-token reuse detection), implemented for `InMemoryRevocation` and `DjangoCacheRevocation`. (#239)
- **CSRF protection for cookie-authenticated routes** - When a JWT backend reads its token from a `cookie`, unsafe-method (state-changing) requests are checked for cross-site origin in Rust (`Sec-Fetch-Site`, falling back to an `Origin`/`Referer` host comparison), closing the gap left by bolt bypassing Django's `CsrfViewMiddleware`. Enabled by default for cookie backends; opt out with `csrf=False`. (#239)
